...
function App() {
  ...
  async function updateCachedShortLinks() {
    const shortLinksCookie = await window.cookieStore.get("redirector_short_links");
  ...
  useEffect( () => {
    updateCachedShortLinks();
  }, []);

  useEffect(() => {
    document.cookie = `redirector_short_links=${JSON.stringify(shortLinks)};max-age=28800;`
  }, [shortLinks]);
